...
// Upgrade NOTE: replaced 'UNITY_PASS_TEXCUBE(unity_SpecCube1)' with 'UNITY_PASS_TEXCUBE_SAMPLER(unity_SpecCube1,unity_SpecCube0)'
...
#if !defined(MY_LIGHTING27_INCLUDED)
    #define MY_LIGHTING27_INCLUDED
    ...
    InterpolatorsVertex  MyVertexProgram(VertexData v)
    {
        ...
        UNITY_TRANSFER_INSTANCE_ID(v, i);
            ...
            //由于我们不在片段程序中这样做，因此没有可用的屏幕空间导数，所以 GPU 无法确定使用哪个 mipmap 级别。
            //我们不能使用 tex2D，相反，我们必须使用 tex2Dlod 来指定一个显式的 mipmap 级别。
            //这是通过提供两个额外的纹理坐标来完成的，
            //第三个是一个未使用的3D 坐标，第四个指定 mip 级别。我们将两者都使用0，有效地不使用 mipmap。
    ...
    */
    ...
    UnityIndirect CreatIndirectLight(Interpolators i,float3 viewDir,SurfaceData surface)
    {
 ...
 */
    ...
    FragmentOutput MyFragmentProgram(Interpolators i)
    {
        ...
        UnityIndirect unity_indirect = CreatIndirectLight(i,viewDir,surface);
